...
E[z(\theta^{(new)})|z_{\mbox{\scriptsize known}}] = \mu(\theta^{(new)}) + \sigma^2_{GP}r'V(z_{\mbox{\scriptsize known}}-M)
\end{equation}
and variance
%\begin{equation*}
\[Var[z(\theta^{(new)})|z_{\mbox{\scriptsize known}}] = \sigma^2_{\mbox{\scriptsize GP}} + \sigma^2_e - \sigma^4_{GP}r'Vr.\]
%\end{equation*}
